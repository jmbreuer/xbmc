...
#define SETTING_SUBTITLE_STRETCH               "subtitles.stretch"
...
void CGUIDialogSubtitleSettings::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  ...
  else if (settingId == SETTING_SUBTITLE_STRETCH)
  {
    ESUBTITLESTRETCH value = static_cast<ESUBTITLESTRETCH>(std::static_pointer_cast<const CSettingInt>(setting)->GetValue());
    appPlayer->SetSubtitleStretch(value);
  }
...
void CGUIDialogSubtitleSettings::InitializeSettings()
{
  ...
  if (SupportsSubtitleFeature(IPC_SUBS_STRETCH))
  {
    ...
    entries.clear();
    entries.push_back(TranslatableIntegerSettingOption(39199, ST_STRETCH_NONE));
    entries.push_back(TranslatableIntegerSettingOption(39200, ST_STRETCH_2425));
    entries.push_back(TranslatableIntegerSettingOption(39201, ST_STRETCH_2524));
  
    AddSpinner(groupSubtitles, SETTING_SUBTITLE_STRETCH, 39198, SettingLevel::Basic, videoSettings.m_subtitleStretch, entries);
  }
